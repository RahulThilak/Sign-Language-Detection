...
# Labels for words and phrases
labels = ["A", "are", "B", "C", "Good", "Hi", "How", "I", "I Love You", "M", "No", "Peace", 
          "Thank", "Welcome", "Who", "Yes", "You", "Please", "Bad", "Sad", "Angry", 
          "Help", "See", "Later", "Water", "Look", "Bathroom", "Sorry", "Happy", "Awesome"]
...
def capture_frames():
    while True:
        ...
        if not success or img is None:
            st.warning("Failed to capture video. Please check your camera.")
            continue  # Skip to the next iteration if the capture failed

        img = cv2.flip(img, 1)  # Flip the image horizontally for a mirror effect
        imgOutput = img.copy()  # Copy the captured frame for output
        ...
        if hands:
            for hand in hands:  # Iterate over all detected hands
                ...
                if imgCrop.shape[0] > 0 and imgCrop.shape[1] > 0:
                    aspectRatio = h / w
                    imgWhite = np.ones((imgSize, imgSize, 3), np.uint8) * 255  # Create a white square background

                    # Dynamically resize based on aspect ratio
                    if aspectRatio > 1:
                        k = imgSize / h
                        wCal = math.ceil(k * w)
                        imgResize = cv2.resize(imgCrop, (wCal, imgSize))
                    ...
                    # Pass the resized image for prediction
                    prediction, index = classifier.getPrediction(imgWhite, draw=False)
...
# Call the capture_frames function to start the video stream
capture_frames()
